...
fn test_first_run_shows_manual_setup_hint() {
    ...
    // Status is exempt from config check, so use "stop" which isn't exempt
    let output = Command::cargo_bin("blackbox")
        .unwrap()
        .env("XDG_CONFIG_HOME", &config_dir)
        .env("XDG_DATA_HOME", &data_dir)
...
#[test]
fn test_standup_lookback_from_config() {
    let tmp = TempDir::new().unwrap();
    let config_dir = tmp.path().join("config");
    let data_dir = tmp.path().join("data");

    // Create config with standup_lookback_days = 1
    let cfg_dir = config_dir.join("blackbox");
...
"#,
    ...
    assert!(output.status.success(), "standup with config lookback=1 should succeed");
    let stdout = String::from_utf8_lossy(&output.stdout);
    // Should NOT say "Today" — should show a date range or "No activity recorded for <range>"
    assert!(!stdout.contains("\"Today\"") && !stdout.contains("**Today"),
        "lookback=1 should not label as Today, got: {}", stdout);
}
...
// --- US-010: Integration tests for blackbox status ---

#[test]
fn test_status_no_db_shows_stopped() {
    let tmp = TempDir::new().unwrap();
    let config_dir = tmp.path().join("config");
    let data_dir = tmp.path().join("data");
    // No config, no DB — status should still work (exempt from config check)
    Command::cargo_bin("blackbox")
        .unwrap()
        .env("XDG_CONFIG_HOME", &config_dir)
        .env("XDG_DATA_HOME", &data_dir)
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Stopped").or(predicate::str::contains("\u{2717}")));
}

#[test]
fn test_status_json_valid_with_expected_keys() {
    let tmp = TempDir::new().unwrap();
    let config_dir = tmp.path().join("config");
    let data_dir = tmp.path().join("data");
    ...
    assert!(json.get("running").is_some(), "missing 'running' key");
    assert!(json.get("pid").is_some(), "missing 'pid' key");
    assert!(json.get("health").is_some(), "missing 'health' key");
    assert!(json.get("events_today").is_some(), "missing 'events_today' key");
}

#[test]
fn test_status_json_no_daemon_running_false_health_red() {
    let tmp = TempDir::new().unwrap();
    let config_dir = tmp.path().join("config");
    let data_dir = tmp.path().join("data");
