...
fn zsh_hook_contains_chpwd() {
    ...
    assert!(
        script.contains("chpwd_functions"),
        "zsh hook must use chpwd_functions"
    );
    assert!(
        script.contains("_blackbox_hook"),
        "zsh hook must define _blackbox_hook"
    );
    assert!(
        script.contains("_notify-dir"),
        "zsh hook must call _notify-dir"
    );
}
...
fn bash_hook_contains_prompt_command() {
    ...
    assert!(
        ...
        "bash hook must use PROMPT_COMMAND"
    );
    assert!(
        script.contains("_BLACKBOX_HOOKED"),
        "bash hook must guard against double-hook"
    );
    assert!(
        script.contains("_notify-dir"),
        "bash hook must call _notify-dir"
    );
}
...
fn fish_hook_uses_on_variable() {
    ...
    assert!(
        ...
        "fish hook must use --on-variable PWD"
    );
    assert!(
        script.contains("_notify-dir"),
        "fish hook must call _notify-dir"
    );
